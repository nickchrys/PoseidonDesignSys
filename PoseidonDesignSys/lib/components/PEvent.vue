...
const formatDate = (date) => {
    ...
    return format(parsedDate, 'MMM. do'); // Example: "Apr. 2nd"
};
...
<template>
    <div :class="giveDesign" @click="handleEventClick" :style="{
        ...
        <!-- Back Icon -->
        <svg v-if="giveDesign.includes('p-event--header') || giveDesign.includes('p-event--header-edit')"
        ...
        <!-- Edit Icon -->
        <svg v-if="giveDesign.includes('p-event--block-planner')" class="pencil-icon" xmlns="http://www.w3.org/2000/svg"
            width="20" height="20" viewBox="0 0 24 24" @click="handleEditClick">
        ...
        <h3 v-else class="p-event__dates">{{ formatDate(startDate) }} - {{ formatDate(endDate) }}</h3>
